...
### エントリポイントとランタイム構成
- **`src/code.ts`** — Figmaプラグインのメインスレッド側エントリ。`figma.showUI(__html__, { width: 280, height: 120, themeColors: true })` で進捗表示用UIを表示する（`visible: false` のヘッドレス起動は廃止）。モジュール読み込み時点では `main()` を呼ばず、UI からの `ui-ready` メッセージを受けて `once` フラグ付きで起動する（UI 側の DOM 準備を待つハンドシェイク）。`main()` は各フェーズ前に `postProgress(step, current?, total?)` で `export-progress` を送りつつ逐次的にデータ取得→トークン生成→`figma.ui.postMessage({ type: "download-zip", ... })` で UI に渡す。同期処理ブロックの前後には `yieldToUi()`（`setTimeout(resolve, 0)` の1tick譲渡）を挟み、postMessage をUIスレッドに描画させる。`main()` が例外を投げた場合は `figma.closePlugin` せず `{ type: "export-error", error: String(e) }` を postMessage する（console.error は維持）。`figma.ui.onmessage` は `isUiToPluginMessage` で型ガードしてから分岐する。
- **`src/progress.ts`** — 進捗ステップ (`PROGRESS_STEPS` = `collections` → `convert` → `text-styles` → `paint-styles` → `effect-styles` → `zip`) とその日本語ラベル (`PROGRESS_STEP_LABELS`)、型ガード `isProgressStep`、重み付き % 計算 `progressPercent(step, current?, total?)` を持つ、Figma SDK / DOM に依存しない共有モジュール。code.ts と src/ui/main.ts の両方から import される。重みは `collections: 70` / `convert・text-styles・paint-styles・effect-styles: 各5` / `zip: 10`（合計100）。current/total を渡すと step 内をサブ進捗として線形補間し、それ以外（total が 0 以下・非有限、current が非有限）は step 開始位置を返す。
- **`src/ui/main.ts`** — UIスレッド側エントリ（TypeScript、DOM lib あり）。スクリプト起動時に `postToPlugin({ type: "ui-ready" })` を送って code.ts 側の `main()` を起動させる。`onmessage` で受け取った `PluginToUiMessage` を `isPluginToUiMessage` で型ガードし、`export-progress` は `PROGRESS_STEP_LABELS` / `progressPercent` を使って進捗ビュー（ラベル＋プログレスバー）を更新、`download-zip` は "zip" ステップの表示に切り替えてから npm 同梱の JSZip（`import JSZip from "jszip"`）で `src/zipEntries.ts` の `buildZipEntries` が組み立てたエントリを zip に詰めてダウンロードし、`export-error` は失敗ビュー（danger色）を表示する。ZIPダウンロード成功後、警告 0 件なら完了ビュー表示のみ、警告ありなら「警告 N 件」＋ `_export-warnings.json` 案内を表示のうえ `download-complete` を送る（ZIP 生成自体が失敗した場合は失敗ビュー表示＋ `{ type: "error" }` を送る）。閉じるボタン押下で `{ type: "close" }` を送る。ZIPファイル名は code.ts 側の `buildZipFilename`（`src/zipFilename.ts`）で組み立てられ、postMessage の `data.zipFilename` として渡される（欠落時は `"figma-export.zip"` にフォールバック）。
- **`src/ui/template.html`** — 進捗ビュー（フェーズラベル＋プログレスバー、id: `progress-view`/`progress-label`/`progress-track`/`progress-fill`）と結果ビュー（初期 `hidden`、id: `result-view`/`result-status`/`result-detail`/`close-button`）を持つテンプレート。CSS は `figma.showUI` の `themeColors` が注入する CSS 変数をフォールバック値付きで使用（例: `var(--figma-color-bg-brand, #0d99ff)`）。`<!-- %UI_SCRIPT% -->` プレースホルダに `build.mjs` がバンドル済み `main.ts` の JS を `<script>` タグとして埋め込み `dist/ui.html` を生成する（生成物のため `dist/` は git 管理外）。
...
- **`src/types/messages.ts`** — code.ts ⇄ UI 間のメッセージ契約。`PluginToUiMessage` は `download-zip` / `export-progress`（`step: ProgressStep`, `current?: number`, `total?: number`） / `export-error`（`error: string`）の union。`UiToPluginMessage` は `ui-ready` / `download-complete` / `close` / `error`（`error: string`）の union。ランタイム型ガード `isPluginToUiMessage` / `isUiToPluginMessage` は参照するフィールドの形まで検証する（`export-progress` は `isProgressStep`、`export-error`/`error` は `error` が string であることを確認。`ui-ready`/`close`/`download-complete` はペイロードなしのため type 判別のみ）。
...
### 起動フローと進捗UI
1. `figma.showUI` 後、UI（`src/ui/main.ts`）が DOM 構築を終えたら `{ type: "ui-ready" }` を送る
2. `figma.ui.onmessage` が `ui-ready` を受けたら `started` フラグで多重起動を防ぎつつ `main()` を開始する
3. `main()` は各フェーズ突入前に `postProgress(step, current?, total?)` で `export-progress` を送る。`collections` ステップは `getCollections` の `onProgress` コールバック経由で 50件チャンクごとに `current`/`total` 付きで細かく進捗を送る。他のステップは開始時に1回送るのみ
4. 完了時: `{ type: "download-zip", data }` を postMessage。UI 側は ZIP 生成後、警告 0 件なら完了ビュー表示のみ、警告ありなら完了ビュー（警告件数＋案内）表示のうえ `download-complete` を送る
5. `figma.ui.onmessage` の `download-complete` 受信時、**警告 0 件なら `figma.closePlugin("エクスポートが完了しました")` で即クローズ**（Figmaのトースト通知）。**警告あり、または `export-error` 発生時は closePlugin せず UI の完了/失敗ビューを表示し続け**、ユーザーが閉じるボタン（`{ type: "close" }`）を押すまで待つ

### `main()` の処理フロー（`src/code.ts`）
0. 冒頭で `setDocumentColorSpace(documentColorSpaceFromProfile(figma.root.documentColorProfile))`（`src/converts/util.ts`）を呼び、`makeColorValue` が出力する `colorSpace` をドキュメント単位で1回だけ注入する（issue #8）
1. `getCollections(onProgress)` でローカル変数を全Collection取得（`src/collections.ts` の `chunk` で variableIds を50件ずつに分割し、チャンク完了ごとに `onProgress(done, total)` を呼ぶ）
...
- **Paint Styles** → `color`（SOLID）/ `gradient`（GRADIENT_LINEAR）。SOLID / GRADIENT とも `paint.opacity ?? 1` が 1 のときのみ `boundVariables.color` のエイリアス参照を採用する。opacity ≠ 1 の場合は参照を破棄して RGBA を焼き込み（SOLID: alpha = opacity、GRADIENT: stop の alpha × opacity）、エイリアスが存在した場合は `kind: "paint-opacity"` の警告を記録する（GRADIENT は 1 paint につき最大1件）。複数paintは `{name}-color-{index}` / `{name}-gradient-{index}` で命名（0始まり）。IMAGE/VIDEO は除外。ColorValue の構築は全変換共通で `makeColorValue`（`src/converts/util.ts`）を使い、alpha === 1 のときは DTCG 既定値（1）に合わせて `alpha` キーを省略する（alpha ≠ 1 の値は丸めずそのまま出力する）。`colorSpace` は `main()` 冒頭で `figma.root.documentColorProfile` から `setDocumentColorSpace` により注入された値（`"srgb"` または `"display-p3"`）を使い、数値変換は行わずラベルのみ出し分ける（issue #8）。
